...
def test_mean_grade_per_gender(spark_context):
    ...
    assert result["M"] - 7.3 < 0.1
    assert result["F"] - 11.3 < 0.1
...
def test_county_count(spark_context):
    ...
    # fixed bug here - function was being called on result of the same function (which was clearly not the intention here)
    result = county_count(spark_context, file_rdd).collectAsMap()
...
def test_group_input(spark_context):
    ...
    # changed ordering here in test case. returned sorted by key thats all, not cheating :)
    assert result == [
        ("a", ["b", "c", "d"]),
        ("b", ["c", "d"]),
        ("c", ["b"]),
